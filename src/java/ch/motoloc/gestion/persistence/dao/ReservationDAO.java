...
//           .append("AND TO_CHAR(reservation.dateDebut, 'DD.MM.YYYY') >= CURRENT_DATE").toString();
    ...
    public List<Reservation> findActive(Client client){
        String request = "SELECT res FROM Reservation res WHERE res.forfait.client = ?1 AND res.isAnnule = FALSE AND res.dateFin > CURRENT_DATE ORDER BY res.dateFin DESC ";
